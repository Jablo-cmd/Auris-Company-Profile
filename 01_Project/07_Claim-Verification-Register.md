...
| Part Two — Services | **Full — line-by-line audited at Stage 8A (2026-09-13)** | Every one of the ten Part Two files (2.0–2.9) was read in full against this register. Found two claims not covered by the blanket per-service fact-callout gate (CVR-010) — added as CVR-041 (2.1) and CVR-042 (2.8). 2.5's fact callout was also resolved in the same session (partial, one service only) — see the CVR-010 update above and CVR-038/CVR-040 |
...
| Part Seven — Credentials | Not yet drafted, **except 7.10 Case studies (early-drafted and audited together, 2026-09-13)** | Every other Part Seven section remains open; 7.10 was drafted ahead of schedule against verified evidence — see Fact Register F1 note. A cross-document consistency risk between 7.10 and the public website's "LOGIOS OS" portfolio entry is tracked at CVR-046 |

**Parts One through Five are now all audited line-by-line** — this does not mean every claim within
them is *verified*; most remain **Gated** or **Unverified** pending evidence — only that coverage of
the copy against this register is complete for those five parts. Part Six and the remainder of Part
Seven remain not yet drafted and will be audited at drafting time, per the working method used since
Stage 7.

### Register tally (as of 2026-09-13, Stage 8A line-by-line audit of Parts Two–Four)

| Status | Count | Rows |
|---|---|---|
| Verified | 11 | CVR-002, 013, 035, 036, 037, 038, 039, 040, 043, 044, 045 |
| Gated (open Fact Register token) | 15 | CVR-001, 003, 004, 005, 010, 011, 014, 015, 016, 017, 018, 022, 024, 026, 028 |
| Unverified (no evidence, no gating token) | 16 | CVR-006, 007, 008, 009, 012, 019, 020, 021, 023, 025, 027, 029, 030, 041, 042, 046 |
| Rejected | 0 | — |
| **Total** | **42** | CVR-001 through CVR-030 (30 rows), plus CVR-035 through CVR-046 (12 rows; IDs 031–034 not used) |
...
when the two are documented as the same product or the public website's claim is corrected.
...
| CVR-010 | 2.1–2.8 (all service spreads) | "Fact callout" proof point per service | A real, named or anonymised delivered example per service | Delivery leadership | Gated ([[PROOF_*]] per service, Fact Register C2) — **partially resolved 2026-09-13, Stage 8A, for 2.5 Web Design and Development only** (Pro Energy Solutions; see CVR-038/CVR-040). Remains Gated for the other seven services (2.1–2.4, 2.6–2.8), each with its own open `[[PROOF_*]]` token | High | — |
...
| CVR-043 | 4.1 Software engineering — "Technologies currently used" | "demonstrated implementation experience with React 18, TypeScript and Vite for front-end application development, evidenced directly in the source code of multiple Auris-developed systems (Sebetsa, Funda360, and a further Auris-built system held confidentially...)" | Direct inspection of `package.json` dependencies and config files (`vite.config.ts`, `tailwind.config.ts`) and in-code framework usage in the Sebetsa and Funda360 repositories | Engineering leadership | **Verified against source repository.** Confirmed 2026-09-13, Stage 8A, by direct inspection of dependency manifests, config files and in-code usage. The claim text is deliberately scoped to demonstrated implementation experience only — it does not claim commercial delivery volume, years of experience or a vendor certification, none of which is asserted | Medium | 2026-09-13 |
| CVR-044 | 4.1 Software engineering — "Code quality regime" | "automated testing... unit and component testing via Vitest, and end-to-end browser testing via Playwright... alongside ESLint and Prettier for static analysis and formatting" | Direct inspection of `vitest.config.ts`, `playwright.config.ts`, and ESLint/Prettier configuration files in the same repositories | Engineering leadership | **Verified against source repository.** Confirmed 2026-09-13, Stage 8A. The same passage explicitly states that "specific coverage-threshold policy and merge review-gate configuration are not evidenced and remain unconfirmed" — the claim does not overreach beyond what was inspected | Medium | 2026-09-13 |
| CVR-045 | 4.6 Data platforms and databases — "Technologies currently used" | "demonstrated implementation experience with PostgreSQL as the primary relational data store, accessed through Supabase — which... also provides authentication..., file storage, and serverless functions running on Deno" | Direct inspection of database migration files, in-code usage (`supabase.storage.from()`, `auth.uid()`) and Deno-based Edge Function source across the same Auris-developed systems | Engineering leadership | **Verified against source repository.** Confirmed 2026-09-13, Stage 8A. The claim is explicitly scoped to demonstrated implementation experience — it does not evidence a managed-hosting arrangement, an uptime commitment, or a database engine choice for any specific client engagement, none of which is claimed | Medium | 2026-09-13 |
| CVR-046 | 7.10 Case studies (Logistics App) vs. public Auris Nexus Technologies website | The public website's portfolio page names a case study "LOGIOS" / "LOGIOS OS"; this profile instead evidences a real, owner-confirmed product named "Logistics App" (Fact Register F10), with no documentary confirmation that the two names refer to the same product | Either (a) the local Logistics App source or its own documentation explicitly confirming "LOGIOS"/"LOGIOS OS" as an official or historical name for it, or (b) confirmation that the two are separate products — followed by correction of whichever public-facing claim is then unsupported | Auris Nexus Technologies (product/marketing) | **Unverified — flagged as a cross-document consistency risk, not gated by an internal token.** Added 2026-09-13, Stage 8A, following the Logistics App owner-confirmation (Fact Register F10) and the pre-existing "LOGIOS OS" exclusion finding (Fact Register F9). This profile's own copy does not use the name "LOGIOS"/"LOGIOS OS" anywhere (7.10 names only "Logistics App") and must not adopt it until this resolves | Medium | — |
